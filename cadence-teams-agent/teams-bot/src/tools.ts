...
export const tools = [
  ...
  { type: 'function', function: { name: 'get_initiative_status', description: 'Get the detailed status of one initiative: result vs target, gap, sufficiency, and the sub-work/activity breakdown. Use when the user names or refers to a specific initiative.', parameters: { type: 'object', properties: { initiative: { type: 'string', description: 'Initiative title or a distinctive part of it, e.g. "laptops" or "logistics cost".' } }, required: ['initiative'] } } },
  { type: 'function', function: { name: 'plan_initiative', description: 'Create a new initiative from a decomposition YOU produce, and assign it to a team. Break the goal into 3-6 sub-works, each with 1-4 concrete activities (each with an hour estimate and a type of self/meeting/call/site). The service distributes activities across the team balanced by load and spreads dates to the deadline. Always confirm the plan with the user before calling this.', parameters: { type: 'object', properties: { title: { type: 'string' }, type: { type: 'string', enum: ['procurement', 'cost', 'onboarding', 'compliance', 'general'] }, objective: { type: 'string' }, deadline: { type: 'string', description: 'ISO date YYYY-MM-DD' }, team: { type: 'string', description: 'Team name to assign to, e.g. "IT Ops".' }, subworks: { type: 'array', items: { type: 'object', properties: { title: { type: 'string' }, activities: { type: 'array', items: { type: 'object', properties: { title: { type: 'string' }, estimateHrs: { type: 'number' }, type: { type: 'string', enum: ['self', 'meeting', 'call', 'site'] } }, required: ['title'] } } }, required: ['title', 'activities'] } } }, required: ['title', 'type', 'team', 'subworks'] } } },
    ...
    try {
      switch (name) {
        ...
        case 'plan_initiative': {
          ...
          const res = await cadence.createInitiative({ ownerId: ctx.userId, title: args.title, type: args.type, objective: args.objective, deadline: args.deadline, teamId: team.id, subworks: args.subworks });
